...
class Spec_Model_Mapper_Mongo extends Spec_Model_Mapper_Abstract
{
	/**
	 *
	 * @var string
	...
	public function __construct($model_class_name = null)
	{
		...
		if (null === $model_class_name)
		{
			throw new Zend_Exception("Model class name don't writed");
		}
	...
	public function upsert(Spec_Model_Interface $model, $safe = FALSE)
	{
		return $this->update($model, $safe, TRUE);
	}
	 ...
	 * @return boolean
	 */
	public function save(Spec_Model_Interface $model)
	{
		if ( ! $model->isCreated())
		{
	...
	{
		$cond = $this->_make_query(array($this->_primary_key_name => $model->getPrimaryKeyValue()));
		return $this->_getCollection()->remove($cond['cond'], array ('safe' => (bool)$safe));
	}
	...
	{
		$condition = array();
		if ($cond instanceof Spec_Model_Mapper_Mongo_Expression)
		{
	...
	{
		#if (!isset(self::$_mongos[$this->_mongo_instance_name]))
		{
			$config = $this->_get_config();
			...
			self::$_mongos[$this->_mongo_instance_name] = new Mongo($connection, $options);
		}
