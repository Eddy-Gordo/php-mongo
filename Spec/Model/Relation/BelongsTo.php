...
class Spec_Model_Relation_BelongsTo extends Spec_Model_Relation
{
	/**
	 * @var Spec_Model_Interface
	 */	
	...
	/**
	 * 
	 * @param Spec_Model_Interface $model
	 * @param string $foreign_class_name
	 * @param array  $options
	 * @param string $property_name
	 */
	public function __construct(Spec_Model_Interface $model, $foreign_class_name, $options, $property_name)
	{		
		...
		{
			throw new Zend_Exception("Class $this->_foreign_class_name is not defined");
		}
		...
		if (!$exists)
		{
			throw new Zend_Exception("Model $this->_local_class_name has no property $this->_local_key");
		}
	}
	 ...
	 * @return Spec_Model_Interface|null
	 */
	public function match($request_params)
	{
		...
		{ 
			...
			{
				throw new Zend_Exception("Object of $this->_foreign_class_name for matched relation $this->_relation_name not found " . implode("=", $fetch_params));
			} 
			else 
			{
	...
	public function get_foreign_object()
	{
		if (null === $this->_foreign_object) 
		{
			...
			if (null === $this->_foreign_object) 
			{
				...
				{
					throw new Zend_Exception("Object of $this->_foreign_class_name is not found by param '$this->_foreign_key = $local_value'");
				} 
				else 
				{
	...
	/**
	 * 
	 * @param Spec_Model_Persistent $object
	 * 
	 * @return void
	 */
	public function set_foreign_object($object)
	{
		...
		{
			throw new Zend_Exception("Invalid parent. Must be instance of {$this->_foreign_class_name}");
		}
	}
	...
	public function set_foreign_objects_to_collection(Spec_Model_Collection $collection, $refresh = false)
	{
		...
		foreach ($collection as $model) 
		{
			...
			{
				throw new Zend_Exception("Invalid local object. Must be instance of {$this->_local_class_name}");
			}
