...
class Spec_Model_Abstract implements Spec_Model_Interface
{
    /**
     * @var bool
     */
    ...
    {
        $reflection = Spec_Model_Reflection::instance($this->_class_name);
    ...
    public function _setupRelations()
    {
        if (!array_key_exists($this->_class_name, self::$_relations_map)) {
            ...
            $doc_block_properties = Spec_Model_Reflection::instance($this->_class_name)->getProperties();

            foreach ($doc_block_properties as $property) {
                // relation
                if ($property->isRelation()) {
                    $relation = Spec_Model_Relation::factory($this, $property);
                    if (null !== $relation) {
                        self::$_relations_map[$this->_class_name][$relation->get_relation_name()] = $relation;
                    }
                    else {
                        throw new Zend_Exception('Wrong relation definition');
                    }
                }
            }
        }
    ...
    public function populate($data)
    {
        ...
        if (!is_array($data)) {
            throw new Zend_Exception("Can't populate data. Must be array or object.");
        }
    ...
    public function __get($property)
    {
        ...
        throw new Zend_Exception('Property '.$property.' does not exist');
    }
    ...
    public function __set($property, $value)
    {
        ...
        if (in_array($property, array_keys($this->_data))) {
            ...
            if (!array_key_exists($property, $this->_data)) {
                throw new Zend_Exception('Setting new entity property ('.$property.') is forbidden');
            }
        }
    }
    ...
    {
        return Spec_Model_Reflection::instance(self::_getClassName())->getProperty($name)->type;
    }
